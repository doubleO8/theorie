...
class ChomskyGrammatik(automaten.Automat):
			...
			for production in self.P[nonterminal]:
				...
				if len(production) > 2:
					...
					if not (self.N.intersection(set(production)) == set(production)):
						self.log.debug("Nicht alle Teile der Produktion sind Terminals. N:%s production:%s; intersection: %s" % (self.N, production, self.N.intersection(set(production))))
						return False
			...
			for production in self.P[terminal]:
				...
				if len(production) > 2:
					...
					if not (self.T.intersection(set(a)) == set(a) ):
						self.log.debug("'a-Teil' der Produktion ist kein Terminalsymbol. T:%s a:%s; intersection: %s" % (self.T, alpha, self.T.intersection(set(a))))
						return False
					if not (self.T.union(self.N).intersection(set(alpha)) == set(alpha) ):
						self.log.debug("'alpha-Teil' der Produktion ist kein Terminal- oder Nonterminalsymbol. T+N:%s a:%s; intersection: %s" % (self.T.union(self.N), alpha, self.T.union(self.N).intersection(set(alpha))))
						return False
		return True
	...
	def machKellerautomat(self):
		...
		for nonterminal in self.P.keys():
			for production in self.P[nonterminal]:
				automat.addRule('s1', kellerautomaten.DeterministischerKellerautomat.EPSILON, nonterminal, 's1', '+'.join(production))
